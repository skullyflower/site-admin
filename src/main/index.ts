...
import { getBlogs, updateBlogInfo, updateBlogPost, deletEntry } from './modules/blogBits'
...
ipcMain.handle('update-blog-post', updateBlogPost)
...
ipcMain.handle('set-config', (_event, config: { pathToSite: string }) => updateAdminConfig(config))

ipcMain.handle('select-site-directory', () => {
  const directory: Promise<OpenDialogReturnValue> = dialog.showOpenDialog({
    properties: ['openDirectory']
  })
  return directory.then((result) => result.filePaths)
})
